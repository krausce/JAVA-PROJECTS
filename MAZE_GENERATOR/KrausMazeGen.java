...
public class KrausMazeGen {
        ...
        private static void makeMazeRecursive(char[][] level, int startX, int startY, int endX, int endY) {
                ...
                if (((endX - startX) < 3) || ((endY - startY) < 3)) {
                        ...
                        int vertPartition = randNumGen(startX, endX);
                        int horPartition = randNumGen(startY, endY);
                        // Draw new vertical partition.
                        for (int y = startY; y < endY; y++)
                                level[y][vertPartition] = ICON_WALL;
                        level[randNumGen(startY + 1, endY - 1)][vertPartition] = ICON_BLANK;
                        // level[randNumGen(horPartition, endY)][vertPartition] = ICON_BLANK;

                        // Draw new horizontal partition.
                        for (int x = startX; x < endX; x++)
                                level[horPartition][x] = ICON_WALL;
                        level[horPartition][randNumGen(startX, endX - 1)] = ICON_BLANK;
                        // level[horPartition][randNumGen(vertPartition, endX)] = ICON_BLANK;

                        makeMazeRecursive(level, startX + 1, startY + 1, vertPartition - 2, horPartition - 2);
                        makeMazeRecursive(level, vertPartition + 1, startY + 1, endX - 2, horPartition - 2);
                        makeMazeRecursive(level, startX + 1, horPartition + 1, vertPartition, endY);
                        makeMazeRecursive(level, vertPartition + 1, horPartition + 1, endX - 2, endY - 2);
                }
