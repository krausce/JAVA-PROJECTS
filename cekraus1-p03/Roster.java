 ...
 * Class: Roster.java																					*
 * DESCRIPTION																							*
 * This class creates creates a roster of Student objects for editing in memory.						*																													*
 * 																										*
...
public class Roster {
	 ...
	 */
	public Roster() {
		setStudentList(mStudentList = new ArrayList<>());
	}
	...
	public Student getStudent(String pLastName) {
		int studentFound = Searcher.search(getStudentList(), pLastName);
		if (studentFound == -1) {
			return null;
		} else {
			return getStudentList().get(studentFound);
		}
	}
